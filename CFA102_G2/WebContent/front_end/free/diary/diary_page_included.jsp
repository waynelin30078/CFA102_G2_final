...
DiaryVO diary = diarySvc.findByDiaryNo(1);
request.setAttribute("diary", diary);

ActivityRecordService actRecordSvc = new ActivityRecordService();
List<ActivityRecordVO> actRecords = actRecordSvc.findByDiaryNo(1);

request.setAttribute("actRecords", actRecords);

ActivityService actSvc = new ActivityService();
request.setAttribute("actSvc", actSvc);



//登入做好再測試
//Integer mno = new Integer((String)session.getAttribute("mno"));



//Date date1 = Date.valueOf(request.getParameter("date"));
//DiaryVO diary = diarySvc.findByDate(2, date1);
//request.setAttribute("diary", diary);
...
<div class="container diary_page">
	...
	<div class="row ">
			<div class="col-xl-12">
				...
				<p><a href="<%=request.getContextPath()%>/front_end/free/diary/diary_calendar_page.jsp">回到飲食日記首頁</a></p>
			</div>
		</div>
		...
		<div class="row  summary">
			...
			<c:if test="<%= diary.getBodyPic().isEmpty()%>">
				...
				<div id="showpic">
  					<img  id="bodyPic" src="<%= request.getContextPath() + "/front_end/free/diary/images/diary0.jpg"%>">
  				</div>
  						
  				<label for="bodyPic_changed" class="buttom btn btn-primary bodyLabel bodyLabel2">選擇照片</label>
  				<label for="submitBodyPic" class="buttom btn btn-primary bodyLabel">確認上傳</label>
        		<input type="file" id="bodyPic_changed" name="bodyPic_changed" style="visibility:hidden;" onchange="previewPic()" >
				<input class="buttom bottom2" id="submitBodyPic" style="visibility:hidden;" type="submit" value="確認上傳">
				<input type="hidden" name="action" value="insert_diaryPic">
				</form>
			</div>  
			</c:if>
			...
			<div class="col-xl-3">
				<form method="post" action="" enctype="multipart/form-data">
				<div id="showpic">
  				<img  id="bodyPic" src="<%= request.getContextPath() + diary.getBodyPic() %>">
  				</div>
        		...
        		<input type="hidden" name="action" value="update_diaryPic">
				</form>
			</div>
			...
			<div class="col-xl-3">
				...
				<div  class="bodyrecord">
					...
					<br>
					<p>身高(cm)：<input type="text" style="height:20px; width: 50%;" id="ht" name="ht" value="<%= (diary.getHt() == 0)? "" : diary.getHt()%>"></p>
					<p>體重(kg)：<input type="text" style="height:20px; width: 50%;" id="wt" name="wt" value="<%= (diary.getWt() == 0)? "" : diary.getWt()%>"></p>
					<p>體脂肪(%)：<input type="text" style="height:20px; width: 50%;" id="bodyFat" name="bodyFat" value="<%= (diary.getBodyFat() == 0)? "" : diary.getBodyFat()%>"></p>
					<p>腰圍(cm)：<input type="text" style="height:20px; width: 50%;" id="wc" name="wc" value="<%= (diary.getWc() == 0)? "" : diary.getWc()%>"></p>
								...
								<tbody>
									...
									<c:forEach var="meal" items="${meals}">	
										<tr>
											...
											<td><a href="<%=request.getContextPath()%>/meal/meal.do?action=delete_meal&mealNo=${meal.mealNo}" class="btn btn-danger btn-xs sharp"><i
		...
		<div class="row">
			...
			運動項目: ${actSvc.findById(actRecord.actNo).actName}<br>
  					...
  					<div class="col">
    					<div class="collapse multi-collapse" id="multiCollapseExample1">
      						<div class="card card-body add-activity">
       						  					...
       						  					<select id ="selectActivity" onchange="showInfo()" name="actName" required>
												       ...
												       <br>
												單位熱量消耗(kcal/kg/hr)
												<input type="text" id="calPerKgHr" onchange="calculateCal()" style="height:20px; width: 50%; margin: 2px 0px;"  name="calPerKgHr" size="2" readonly><br>
												輸入體重(kg)<br>
												<input type="number" onchange="calculateCal()" style="height:20px; width: 50%; margin: 2px 0px;" id="actWt" name="wt" size="2"><br>
												輸入運動時間(hr)<br>
												<input type="number" onchange="calculateCal()" style="height:20px; width: 50%; margin: 2px 0px;" id="actHr" size="2"><br>
												消耗熱量(kcal)<br>
												<input type="text"  style="height:20px; width: 50%; margin: 2px 0px;" id="calBurn" size="2" readonly><br>
...
<script src="<%=request.getContextPath()%>/Innap-template/vendor/jquery-nice-select/js/jquery.nice-select.min.js"></script>

...
List<DiaryVO> diaries = diarySvc.findByMember(3);
...
<body>
	<!-- *************每一頁body最前面都要include這個header連結******************* fc-widget-content fc-thu fc-past
  	...
  	$( "#nutrition-diary" ).fullCalendar({
  	    ...
  	    dayClick: function(date) {	    	
  	    	...
  	    	if('${date}'.includes(date.format())){
  	    		...
  	    		window.location="<%=request.getContextPath()%>/front_end/free/diary/diary_page_included.jsp?date=" + date.format();
				//diary_page_included.jsp?mno=${member.mno}&date=date.format()																
  	    		...
  	    		if (yes && date.format()<=current_date) {
  	    			window.location="<%=request.getContextPath()%>/diary/diary.do?date=" + date.format();
  		...
  		events: [ // 事件
  			<c:forEach var="diary" items="${diaries}" varStatus="s">
  				...
  				{ // 事件(設定連結)
	  				title: "查看飲食日記",
	  				url: "<%=request.getContextPath()%>/front_end/free/diary/diary_page_included.jsp?date=${diary.diaryDate}",

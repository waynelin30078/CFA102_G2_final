...
<%
						Integer oringin_dstate=(Integer)request.getAttribute("oringin_dstate");
	//     List<DieticianVO> list = (List<DieticianVO>)session.getAttribute("list");
%>
<jsp:useBean id="list" scope="session"
	type="java.util.List<DieticianVO>" />
...
<body id="page-top">
	...
	<div class="container-fluid">
		...
		<table id="table-1">
			<tr>
				<td>
				<c:if test="${oringin_dstate==0}">
					<h3>所有<span style="color:red">未</span>審核營養師會員資料</h3>
					</c:if>
						<c:if test="${oringin_dstate==1}">
					<h3>所有<span style="color:red">通過</span>審核營養師會員資料</h3>
					</c:if>
						<c:if test="${oringin_dstate==2}">
					<h3>所有<span style="color:red">未通過</span>審核營養師會員資料</h3>
					</c:if>
					<h4>
						<a
					...
					<td>
						<FORM METHOD="post"
							...
							<input type="submit"> <input type="hidden"
								...
								type="hidden" name="self" value="selfOne"> <input
								type="hidden" name="origin_dstate" value="${dieticianVO.dstate}">

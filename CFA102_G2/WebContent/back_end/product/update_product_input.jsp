			...
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do" name="form1" enctype="multipart/form-data">
				<table class="table table-striped custab">
					<tr>
						...
						<td><%=productVO.getPno()%></td>
					</tr>
					<tr>
						<td>商品類別名稱:</td>
						<td><input type="TEXT" name="categoryName" size="45" value="<%=productVO.getCategoryName()%>" /></td>
					</tr>
					<tr>
						<td>商品名稱:</td>
					...
					<tr>
						<td>商品單價:</td>
						<td><input type="TEXT" name="pprice" size="45" value="<%=productVO.getPprice()%>" /></td>
					</tr>
					<tr>
						<td>商品描述:</td>
						<td><textarea id="pinfo" name="pinfo" rows="4" cols="42" maxlength="500" ><%=productVO.getPinfo()%></textarea></td>
					</tr>
					<tr>
						<td>商品數量:</td>
						<td><input type="TEXT" name="pquantity" size="45" value="<%=productVO.getPquantity()%>" /></td>
					</tr>
					<tr>
						<td>商品上架日期:</td>
					...
					<tr>
						...
						<td>
							...
							<input type="hidden" name="orgImage1" value="${productVO.pimage1}" >
							<c:choose>
								<c:when test="${productVO.pimage1.trim() == null || productVO.pimage1.trim().length() == 0}">
									<img id="pimage1" class="pic" src="${productVO.pimage1}">
								</c:when>
								<c:otherwise>   
									<img id="pimage1" class="pic" src="<%=request.getContextPath()%>${productVO.pimage1}"/>
  								</c:otherwise>
							</c:choose>
						</td>
					</tr>
					<tr>
						<td>商品圖片2:</td>
						<td>
							<input type="file" name="pimage2" accept="image/*" onchange="document.getElementById('pimage2').src = window.URL.createObjectURL(this.files[0])" />
							<c:choose>
								<c:when test="${productVO.pimage2.trim() == null || productVO.pimage2.trim().length() == 0}">
									<img id="pimage2" class="pic" src="${productVO.pimage2}">
								</c:when>
								<c:otherwise>   
									<img id="pimage2" class="pic" src="<%=request.getContextPath()%>${productVO.pimage2}"/>
  								</c:otherwise>
							</c:choose>
						</td>
					</tr>	
					<tr>
						<td>商品圖片3:</td>
						<td>
							<input type="file" name="pimage3" accept="image/*" onchange="document.getElementById('pimage3').src = window.URL.createObjectURL(this.files[0])" />
							<input type="hidden" name="orgImage3" value="${productVO.pimage3}" >
							<c:choose>
								<c:when test="${productVO.pimage3.trim() == null || productVO.pimage3.trim().length() == 0}">
									<img id="pimage3" class="pic" src="${productVO.pimage3}">
								</c:when>
								<c:otherwise>   
									<img id="pimage3" class="pic" src="<%=request.getContextPath()%>${productVO.pimage3}"/>
  								</c:otherwise>
							</c:choose>
						</td>
					</tr>
...
$('#f_date1').datetimepicker({
   ...
   value: '<%=productVO.getPonDate()%>', // value:new Date(),
...
$('#f_date2').datetimepicker({
   ...
   step: 1,                //step: 60 (這是timepicker的預設間隔60分鐘)
   format:'Y-m-d',         //format:'Y-m-d H:i:s',
   value: '<%=productVO.getPoffDate()%>', // value:new Date(),
   //disabledDates:        ['2017/06/08','2017/06/09','2017/06/10'], // 去除特定不含
   //startDate:	            '2017/07/10',  // 起始日
   minDate: '-1970-01-01' // ,去除今日(不含)之前
   //maxDate:               '+1970-01-01'  // 去除今日(不含)之後

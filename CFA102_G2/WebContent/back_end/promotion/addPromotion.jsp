...
<body id="page-top">
	...
	<div class="container-fluid">
		<h1 class="h3 mb-4 text-gray-800">新增商品優惠活動</h1>
    	...
    	<div class="row col-md-12 col-md-offset-2 custyle">
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/promotion/promotion.do" name="form1">    		
    			<table class="table table-striped custab">
					<tr>
						<td>優惠活動名稱:</td>
						<td><input type="TEXT" name="promName" size="50" value="<%= (promotionVO==null)? "" : promotionVO.getPromName()%>" /></td>
					</tr>
					<tr>
						<td>開始日期:</td>
						<td><input name="promStartDate" id="f_date1" type="text"></td>
					</tr>
					<tr>
						<td>結束日期:</td>
						<td><input name="promEndDate" id="f_date2" type="text"></td>
					</tr>
				</table>
				<br>
...
<% 
  ...
  try {
	  promStartDate = promotionVO.getPromStartDate();
	  promEndDate = promotionVO.getPromEndDate();
   } catch (Exception e) {
	   promStartDate = new java.sql.Date(System.currentTimeMillis());
	   promEndDate = java.sql.Date.valueOf("9999-12-31");
   }
%>
...
<script>
        ...
        $('#f_date1').datetimepicker({
		   ...
		   value: '<%=promStartDate%>', // value:   new Date(),
        ...
        $('#f_date2').datetimepicker({
	       ...
	       step: 1,                //step: 60 (這是timepicker的預設間隔60分鐘)
	       format:'Y-m-d',         //format:'Y-m-d H:i:s',
		   value: '<%=promEndDate%>', // value:   new Date(),
           //disabledDates:        ['2017/06/08','2017/06/09','2017/06/10'], // 去除特定不含
           //startDate:	            '2017/07/10',  // 起始日
           minDate:'-1970-01-01' // ,去除今日(不含)之前
           //maxDate:               '+1970-01-01'  // 去除今日(不含)之後
